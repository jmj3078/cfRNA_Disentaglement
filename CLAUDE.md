...
./Modeling/CV_Results/ : 엔진 CV·진단 출력 = config.CV_RESULTS_DIR. cv_stats.csv(per-gene held-out calibration) · cv_zscores.pkl · cv_ppc.pkl(per-point y/mu/sigma) · cv_summary_by_stage.csv · ppc_summary_stats.csv · Figures/(cv_diagnostics.py 산출). Z 행렬은 여기 아님(→Z_scores/)
